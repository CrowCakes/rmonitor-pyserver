...
class ThreadedServer(object):
    ...
    def listenToClient(self, connection, address, cnx, delivery_print, connected_list):
		...
		#print login_reply
			...
			HandleQuery_UserManagement(user_option, cursor, connection, cnx, insert_data)
		...
		# done with everything, close connections
        try:
            cnx.close()
        except Exception as err:
            pass
...
def HandleQuery(user_option, cursor, connection, cnx, insert_data=[]):
	results = ""
...
def HandleQuery_UserManagement(option, sqlcursor, client_connection, sql_connection, insert_data=[]):
	# all the login user pairs
	if (option == "ViewAllLogin"):
		cursor.execute(make_query(option+'.sql'))
		ViewAllLogin(sqlcursor, client_connection)

	# produces only one login user pair
	elif (option == "ViewPersonalLogin"):
		user_option_data = {'username': insert_data[0]}
		cursor.execute(make_query(option+'.sql'), user_option_data)
		ViewAllLogin(sqlcursor, client_connection)

	elif (option == "ViewLogin"):
		user_option_data = {'username': insert_data[0]}
		cursor.execute(make_query(option+'.sql'), user_option_data)
		ViewLogin(sqlcursor, client_connection)

	elif (option == "Suspend"):
		user_option_data = {'role': "Suspend",
							'username': insert_data[0]}
		client_connection.sendall(ChangeQuery(user_option_data, "ChangeRole"))
	...
	# generate the hash first before inserting into database
		...
		if auth != "":
			...
			client_connection.sendall(ChangeQuery(user_option_data, "ChangePassword"))
		else:
			client_connection.sendall("Error: database password and your old password don't match")

	# generate the hash first before inserting into database
	elif (option == "ChangeUserPassword"):
		new_pass = getDigest(insert_data[0])
		user_option_data = {'password': new_pass[1],
							'salt': new_pass[0],
							'username': insert_data[1]}
		client_connection.sendall(ChangeQuery(user_option_data, "ChangeUserPassword"))
		...
		user_option_data = {'username': insert_data[0],
							...
							'salt': new_pass[0],
							'role': insert_data[2]}
		client_connection.sendall(ChangeQuery(user_option_data, "InsertNewUser"))